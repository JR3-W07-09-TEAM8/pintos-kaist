...
void thread_wakeup(int64_t global_ticks)
{
	...
	struct list_elem *curr_point = list_begin(&sleep_list);
	while (curr_point != list_end(&sleep_list))
	{
		struct thread *curr_thread = list_entry(curr_point, struct thread, elem);
		if (global_ticks > curr_thread->wakeup_tick)
		{
			curr_point = list_remove(curr_point);
			thread_unblock(curr_thread);
		}
		else
		{
			curr_point = list_next(curr_point);
		}
	}
	intr_set_level(old_level);
}
